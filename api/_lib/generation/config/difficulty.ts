...
Требования:
- СПП с несколькими придаточными + знаки при причастных/деепричастных оборотах (7-8кл) внутри придаточных
- Сложные синтаксические конструкции, где нужно учесть и запятые при оборотах, и при союзах, и при вводных словах
- Стилистика (определение стиля) + орфография (Н/НН, НЕ, ПРЕ/ПРИ) в тексте
- Часть заданий должна комбинировать: пунктуацию сложного предложения + обособление из 7-8кл + орфографию из 5-6кл

Примеры заданий:
- "Расставь ВСЕ знаки, вставь буквы: 'Когда я вошёл в комнату где горел написа_ый масл_ыми красками пр_красный пейзаж все замолчали'" (СПП + причастный оборот + Н/НН + ПРЕ/ПРИ)
- "Составь схему, определи все виды придаточных: 'Он сказал что если погода будет хорошей то мы поедем на озеро которое расположе_о (не)далеко'" (3 придаточных + орфография)
- "Определи стиль текста, найди все орфограммы: 'Исследование показало что (не)достаточно изуче_ые районы таят в себе немало тайн'" (стилистика + НЕ + НН)`,

    hard: `Уровень: ПОВЫШЕННЫЙ (9-11 класс)
Требования:
- Задания формата ЕГЭ (8, 21, 25-27), синтезирующие ВСЮ программу 1-11 классов
- Сложные случаи пунктуации: стык союзов + обороты + вводные + однородные в одном тексте
- Анализ текста: проблематика + позиция автора + средства выразительности + все орфограммы
- Задания ОБЯЗАНЫ синтезировать: пунктуацию 5-11кл + орфографию 1-11кл + морфологию + синтаксис + стилистику

Примеры заданий:
- "Найди ВСЕ ошибки (грамматические + пунктуационные + орфографические): 'Подъезжая к (не)знакомому городу, освящё_ому закатным солнцем и нас охватило чувство что мы (не)зря проделали этот путь'" (грамматика деепричастия + НЕ + НН + пунктуация СПП + стык союзов)
- "Расставь знаки и объясни каждый: 'Да я конечно помню этот день когда усталые но счастливые мы вернулись из похода о котором мечтали весь год'" (вводное + обращение + однородные + СПП + определительное)
- "В тексте найди: 1 грамматическую ошибку, 3 орфограммы (объясни), все средства выразительности, определи проблему и позицию автора" (полный синтез курса)`,
  }
  return prompts[level]
}
