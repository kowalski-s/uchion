...
СООТВЕТСТВИЕ КЛАССУ (7+ класс):
- open_question без текстового фрагмента для анализа (т.е. абстрактный вопрос без конкретного текста/примера) → BAD_FORMULATION (warning)
- matching с одним словом в правом столбце (вместо текстового фрагмента или развёрнутого примера) → BAD_FORMULATION (warning)
- Для 9+ класса: любое задание, которое мог бы решить ученик 6 класса → BAD_FORMULATION (warning)
