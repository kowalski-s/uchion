...
Конспект ДОЛЖЕН БЫТЬ СТРУКТУРИРОВАН НА 3–4 ОТДЕЛЬНЫХ АБЗАЦА (разделять пустой строкой).

СТРУКТУРА:
1. Определение и суть темы.
2. Основное правило или объяснение (детали).
3. Примеры (ОБЯЗАТЕЛЬНО отдельным абзацем).

ВАЖНОЕ ОФОРМЛЕНИЕ:
- ОБЯЗАТЕЛЬНО выделять **жирным шрифтом** ключевые термины, правила или формулы.
- Если есть примеры, они должны начинаться с новой строки.
- НЕ писать сплошным текстом. Используй разрывы строк (двойной Enter).

Пример стиля:
«**Союз И** соединяет слова или части предложения, выражая добавление. **Союз А** показывает противопоставление или смену действия.

Пример:
Я пошел в лес, **а** брат остался дома.

**Союз НО** выражает противопоставление с более резким контрастом.»

ЗАПРЕЩЕНО:
- Использовать списки (bullet points).
- Писать сплошной "стены текста".
- Делать конспект слишком коротким (менее 3 абзацев).
...
ДОПОЛНИТЕЛЬНО ДЛЯ МИНИ-ТЕСТА (ОРФОГРАФИЯ/ПУНКТУАЦИЯ):
- Предложения в примерах должны соответствовать нормам школьной пунктуации.
- Если союзы И, А, НО подставляются в готовое предложение с пропуском, не добавляй лишних запятых.
- Не используй запятую между однородными сказуемыми с союзом «и» при одном подлежащем (например, правильно: «Он читает и пишет»).
...
=====================================================================
📌 ЖЁСТКОЕ ТРЕБОВАНИЕ К ТЕХНИЧЕСКОМУ ФОРМАТУ ВЫВОДА:
=====================================================================

Ты ОБЯЗАН выводить результат ровно в следующей структуре и с ЭТИМИ заголовками, БЕЗ любых отклонений, дополнительных слов, пустых строк выше или ниже:
...
Запрещено:
- менять названия этих заголовков,
- добавлять комментарии, пояснения или текст перед/после них,
- изменять отступы,
- вставлять пустой блок или пропускать хотя бы один раздел.

Если хотя бы один раздел отсутствует или назван иначе → считается ошибкой, вывод должен быть перегенерирован внутренне до полного соответствия формату.
`
...
class OpenAIProvider implements AIProvider {
  ...
  async generateWorksheet(params: GenerateParams): Promise<Worksheet> {
    ...
    // We do NOT call generatePrompt(params) here anymore because that was for the JSON-mode prompt.
    // Instead, we construct the user prompt that matches our new "TEXT output" system prompt.
    const userPrompt = `Сгенерируй рабочий лист.
