...
const OPEN_TASK_TYPES = [
  { id: 'open_question', label: 'Открытый вопрос' },
  { id: 'matching', label: 'Соотнесение' },
  { id: 'fill_blank', label: 'Вставка пропуска' },
]

const TEST_TASK_TYPES = [
  { id: 'single_choice', label: 'Единственный выбор' },
  { id: 'multiple_choice', label: 'Множественный выбор' },
]
...
const RegenerateButton = ({
  ...
  return (
    <div className="relative print:hidden" ref={dropdownRef}>
      ...
      {isOpen && (
        <div className="absolute right-0 top-8 z-50 w-52 rounded-lg border border-gray-200 bg-white shadow-lg py-1">
          ...
          {taskTypes.map((type) => (
            <button
              key={type.id}
              onClick={() => {
                onRegenerate(index, type.id, isTest)
                setIsOpen(false)
              }}
          ...
          <section className="flex flex-col">
            ...
            <div className="flex flex-col gap-6">
              {worksheet.assignments.map((task, i) => {
                ...
                const isThisRegenerating = !!regeneratingIndex && !regeneratingIndex.isTest && regeneratingIndex.index === i

                return (
                  <div key={i} className={`task-block break-inside-avoid relative ${i === 0 ? 'mt-2' : ''} ${isEditMode ? 'bg-indigo-50/20 p-4 rounded-xl border border-indigo-100' : ''}`}>
                    {/* Regenerate button */}
                    <div className="absolute right-1 top-1 print:hidden">
                      <RegenerateButton
                        index={i}
                        isTest={false}
                    ...
                    {isThisRegenerating && (
                      <div className="absolute inset-0 bg-white/70 rounded-xl flex items-center justify-center z-10">
                        <div className="flex items-center gap-2 text-indigo-600">
                          ...
                          <span className="text-sm font-medium">Перегенерация...</span>
                        </div>
                      </div>
                    )}
            ...
            <section className="h-full flex flex-col">
              ...
              <div className="grid gap-6">
                {worksheet.test.map((q, i) => {
                  ...
                  return (
                  <div key={i} className={`rounded-xl border bg-white p-5 shadow-sm break-inside-avoid relative print:border print:border-gray-300 print:shadow-none print:p-4 ${isEditMode ? 'border-indigo-200 bg-indigo-50/20' : 'border-gray-100'}`}>
                    {/* Regenerate button */}
