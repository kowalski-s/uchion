...
import { fixTask, MAX_FIXES_PER_GENERATION } from './task-fixer.js';
function collectTasksWithErrors(...agentResults) {
    const errorMap = new Map();
    for (const agentResult of agentResults) {
        for (const taskResult of agentResult.tasks) {
            if (taskResult.taskIndex < 0)
                continue;
...
// Orchestrator
// =============================================================================
export async function runMultiAgentValidation(tasks, params, options = { autoFix: true }) {
    ...
    const allIssues = [];
    const errorTaskIndices = new Set();
    for (const agentResult of agents) {
        for (const taskResult of agentResult.tasks) {
            if (taskResult.taskIndex < 0)
                continue; // Skip agent-level warnings
            for (const issue of taskResult.issues) {
                allIssues.push({
    ...
    if (options.autoFix && problemTasks.length > 0) {
        ...
        for (const { taskIndex, issues } of toFix) {
            const result = await fixTask(tasks[taskIndex], issues[0], // Use the first (most critical) issue
            params);
...
export { fixTask } from './task-fixer.js';
