...
ПРАВИЛО ГЕНЕРАЦИИ ЗАДАНИЙ С ОШИБКАМИ
------------------------------------
Если создаётся задание формата «Найди и исправь ошибку» (тип 3), ты ОБЯЗАН:

1. Явно встроить ошибку в предложение, связанную с темой урока.
2. Выполнить самопроверку:
   - Содержится ли ошибка?
   - Ошибка соответствует теме?
   - Ошибка однозначно корректируется?
...
6. Ошибка должна быть реальной, типичной для ученика данного класса.
7. Нельзя использовать сложность выше класса.

Перед выводом задания сделай внутренний чек:
CHECKLIST:
- [ ] Есть ли ошибка?
- [ ] Ошибка соответствует теме?
- [ ] Предложение корректно исправляется?
- [ ] Ошибка НЕ является спорной?

Если хоть один пункт = «нет», перегенерировать.
