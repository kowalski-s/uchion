...
export function extractWorksheetJsonFromResponse(response: ChatCompletionResponse): WorksheetJson {
  ...
  if ('output_text' in response && typeof response.output_text === 'string' && response.output_text) {
    ...
    if (firstBrace !== -1 && lastBrace !== -1 && lastBrace > firstBrace) {
      raw = raw.slice(firstBrace, lastBrace + 1)
      if (raw) return JSON.parse(raw) as WorksheetJson
    }
  }
...
export function parseValidatorOutput(outputText: string): ValidationResult {
...
export function analyzeValidationIssues(issues: string[]): ValidationIssueAnalysis {
...
export function buildWorksheetTextFromJson(json: WorksheetJson): string {
  const parts: string[] = []
  parts.push('ASSIGNMENTS:')
  for (const a of json.assignments || []) parts.push(`${a.index}) ${a.text}`)
  parts.push('')
  parts.push('TEST:')
...
import { WORKSHEET_BLOCKS_PATCH_SCHEMA } from './schema.js'
...
export async function regenerateProblemBlocks(params: {
  ...
  if (needAssignments) partial.assignments = (original.assignments || []).filter(a => analysis.invalidAssignments.includes(a.index))
  if (needTests) partial.test = (original.test || []).filter(t => analysis.invalidTests.includes(t.index))
