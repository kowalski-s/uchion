# AI Worksheet Generator

Веб-приложение для автоматической генерации рабочих листов по школьным предметам (1–4 классы).
Сервис создаёт:
- краткий конспект,
- 3–5 заданий разного формата,
- блок вопросов,
- PDF-файл для скачивания.

Основано на OpenAI (gpt-5-mini + gpt-4.1-mini) и собственном backend API.

---

## 🚀 Возможности

- **OAuth 2.0 аутентификация** через Google и Yandex с PKCE защитой.
- Генерация рабочих листов по теме урока.
- Поддержка предметов: **математика**, **русский язык**.
- Поддержка классов: **1–4**.
- Строго структурированный формат JSON → PDF.
- Быстрое и минималистичное SPA-приложение.
- PDF генерируется на сервере «на лету».
- Полностью защищённая работа с API-ключами.
- Лимиты генерации для пользователей (5 листов в день).

---

## 🧩 Архитектура

Проект состоит из:

### **Frontend**
- React + TypeScript
- Vite
- Tailwind CSS + shadcn/ui
- React Hook Form + Zod
- React Query
- React Router для навигации

### **Backend**
- Vercel Serverless Functions (Node.js + TypeScript)
- Endpoint `/api/generate` для генерации листов
- OAuth endpoints: `/api/auth/google/*`, `/api/auth/yandex/*`
- Модуль `AIProvider` для взаимодействия с OpenAI
- PDF-генерация через pdfkit

### **Database**
- PostgreSQL (Supabase)
- Drizzle ORM
- Таблицы: `users`, `refresh_tokens`

### **Authentication**
- Custom OAuth 2.0 реализация с PKCE
- JWT токены в httpOnly secure cookies
- Access token: 1 час, Refresh token: 7 дней с ротацией
- CSRF защита через state параметр
- Отзыв токенов через базу данных

### **AI**
- Generation: OpenAI (gpt-5-mini)
- Validation: OpenAI (gpt-4.1-mini)
- OpenRouter (возможность легко переключать модели)

Полная архитектура → см. `/docs` (architecture-overview, tech-stack, api-design и т.д.)

---

## 📁 Структура проекта

```
/docs                        # документация проекта
  architecture-overview.md
  tech-stack.md
  api-design.md
  security-and-performance.md
  ux-ui-guidelines.md
  scaling-plan.md
  prompts-ai.md

/src
  /components                # UI-компоненты
  /pages                     # страницы SPA (LoginPage, GeneratePage, WorksheetPage)
  /lib                       # утилиты, валидации, API-клиенты
    auth.tsx                 # React контекст аутентификации
    api.ts                   # API клиент
  main.tsx

/api                         # Vercel Serverless Functions
  generate.ts                # генерация листов
  /auth
    /google                  # Google OAuth endpoints
      redirect.ts
      callback.ts
    /yandex                  # Yandex OAuth endpoints
      redirect.ts
      callback.ts
    me.ts                    # получение данных пользователя
    logout.ts                # выход из системы
    refresh.ts               # обновление токенов
  /_lib
    /auth                    # OAuth и JWT утилиты
      oauth.ts               # state, PKCE, обмен кода
      tokens.ts              # JWT создание/проверка
      cookies.ts             # управление cookies
      middleware.ts          # проверка аутентификации
    /ai                      # AI-специфичные модули
    ai-provider.ts           # абстракция AI провайдера
    pdf.ts                   # генерация PDF

/db
  schema.ts                  # Drizzle ORM схемы
  index.ts                   # подключение к БД
  /migrations                # SQL миграции

/scripts
  smoke-generate.ts          # тесты генерации
  run-migration.ts           # запуск миграций

/public                      # статика

CLAUDE.md                    # инструкции для Claude Code
README.md
.gitignore
```

---

## ⚙️ Подготовка окружения

### 1. Установка зависимостей:
```bash
npm install
```

### 2. Создание файла окружения:

Создай `.env.local` для локальной разработки:

```bash
# Database
DATABASE_URL=postgresql://user:password@host:port/database

# AI Provider
OPENAI_API_KEY=ваш_ключ
AI_PROVIDER=dummy                # 'dummy' для разработки или 'openai' для продакшена
UCHION_VECTOR_STORE_ID=опционально  # для RAG контекста

# Authentication
AUTH_SECRET=случайная_строка_32+_символов  # для подписи JWT (сгенерируй через openssl rand -base64 32)
APP_URL=http://localhost:3000              # URL приложения

# Google OAuth
GOOGLE_CLIENT_ID=ваш_google_client_id
GOOGLE_CLIENT_SECRET=ваш_google_client_secret

# Yandex OAuth (опционально)
YANDEX_CLIENT_ID=ваш_yandex_client_id
YANDEX_CLIENT_SECRET=ваш_yandex_client_secret
```

Для продакшена (Vercel) задайте переменные в настройках проекта.

### 3. Настройка OAuth приложений:

**Google Cloud Console:**
1. Создай проект в [Google Cloud Console](https://console.cloud.google.com)
2. Включи Google+ API
3. Создай OAuth 2.0 Client ID
4. Добавь redirect URI: `http://localhost:3000/api/auth/google/callback` (для разработки)
5. Добавь redirect URI: `https://твой-домен.vercel.app/api/auth/google/callback` (для продакшена)

**Yandex OAuth:**
1. Создай приложение в [Yandex OAuth](https://oauth.yandex.ru)
2. Добавь callback URL: `http://localhost:3000/api/auth/yandex/callback` (для разработки)
3. Добавь callback URL: `https://твой-домен.vercel.app/api/auth/yandex/callback` (для продакшена)

### 4. Настройка базы данных:

```bash
# Запусти миграции
npm run migration
```

### 5. Запуск проекта:

```bash
# Для тестирования только frontend
npm run dev

# Для тестирования с API (рекомендуется)
vercel dev
```

---

## 🛠 Принципы безопасности

- **OAuth 2.0 с PKCE** - защита от перехвата authorization code
- **CSRF защита** - случайный state параметр (32 байта)
- **JWT в httpOnly cookies** - защита от XSS атак
- **Refresh token rotation** - каждое обновление создаёт новый токен
- **Отзыв токенов** - возможность аннулировать сессии через БД
- **API-ключи только на backend** - никогда не попадают в браузер
- **Валидация входящих данных** - Zod схемы на всех endpoints
- **JSON-ответ ИИ проверяется строгой схемой**
- **PDF генерируется на сервере** - безопасно для клиента

Полная модель безопасности → `/docs/security-and-performance.md`.

---

## 🔮 Roadmap

Кратко:

- ✅ MVP (генерация + PDF)
- ✅ OAuth аутентификация (Google + Yandex)
- ✅ База данных (PostgreSQL + Drizzle)
- ✅ Лимиты генерации
- 🔄 Улучшение качества материалов
- 📋 История сгенерированных листов
- 💳 Монетизация
- 🚀 Переход на VPS
- 📚 Расширение предметов

Полная дорожная карта → `/docs/roadmap.md`.

---

## 🧪 Тестирование

```bash
# Smoke tests с dummy provider (быстро, бесплатно)
npm run smoke

# Smoke tests с реальным OpenAI (стоит денег)
SMOKE_REAL_AI=true npm run smoke
```

**Тестовые фреймворки:**
- Unit: Vitest/Jest
- Integration: Supertest
- E2E: Playwright/Cypress

Тестовые промпты → `/docs/prompts-ai.md`.

---

## 📝 Разработка

Полное руководство для разработчиков и AI ассистентов → `CLAUDE.md`

---

## 📄 Лицензия

Проприетарное ПО. Все права защищены.
