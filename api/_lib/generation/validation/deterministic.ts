...
// The generated task shape from ai-provider.ts
...
function validateSchema(task: GeneratedTask, index: number, errors: ValidationError[]): void {
  const schemaMap: Record<TaskTypeId, z.ZodSchema> = {
...
function validateMathNumbers(
  ...
  const gradeConfig = getGradeConfig(subject, grade as any)
...
function extractQuestionText(task: GeneratedTask): string {
  switch (task.type) {
    case 'single_choice':
    case 'multiple_choice':
    case 'open_question':
...
function gatherTaskText(task: GeneratedTask): string {
  const parts: string[] = []
