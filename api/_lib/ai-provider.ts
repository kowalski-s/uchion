...
ДОПОЛНИТЕЛЬНЫЕ ЖЁСТКИЕ ОГРАНИЧЕНИЯ:

1) МОДЕЛИ ЗАПРЕЩЕНО:
- вводить новые термины, не используемые в ФГОС начальной школы
(например: «оттенок неожиданности», «резкая контрастность» и т.п.);
- усложнять объяснение сверх программы;
- добавлять художественные метафоры, не связанные с учебной задачей.

2) ЕСЛИ ЗАДАНИЕ ТРЕБУЕТ ТЕКСТ (например: «Найди союзы в тексте»),
модель ОБЯЗАНА сама создать короткий текст (1–2 предложения), строго подходящий под уровень класса.

3) КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО:
- разбивать одно задание на подпункты (а), б), в));
- давать несколько мини-заданий внутри одного;
Каждое задание 1–4 должно быть ОДНИМ действием.

4) В МИНИ-ТЕСТЕ:
- Варианты A, B, C не могут повторяться;
- Между вариантами не допускается пересечение (например, два ответа «И»);
- Все варианты должны быть разными и корректными.

5) В МАТЕМАТИКЕ:
- Все выражения в заданиях должны быть выполнимы вручную детьми 1–4 класса;
- Ошибки должны быть ПРАВДИВЫМИ: математически некорректными, но реалистичными.

6) В РУССКОМ ЯЗЫКЕ:
- Нельзя вводить термины, которые относятся к 5–6 классу и выше;
- Все примеры должны иллюстрировать ТОЧНО ТЕМУ урока.

7) ПЕРЕД ВЫВОДОМ:
- проверить, что НИ ОДИН вариант теста не повторяется дважды;
- проверить, что каждое задание содержит СМЫСЛОВОЕ ДЕЙСТВИЕ для ученика.
`
