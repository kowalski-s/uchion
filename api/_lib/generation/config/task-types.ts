...
export type TaskTypeId =
  ...
  | 'fill_blank'      // Вставка пропущенного слова
...
export const MultipleChoiceSchema = z.object({
  ...
  options: z
    .array(z.string().min(1, 'Вариант ответа не может быть пустым'))
    .min(4, 'Минимум 4 варианта ответа')
...
export type SingleChoiceTask = z.infer<typeof SingleChoiceSchema>
export type MultipleChoiceTask = z.infer<typeof MultipleChoiceSchema>
export type OpenQuestionTask = z.infer<typeof OpenQuestionSchema>
export type MatchingTask = z.infer<typeof MatchingSchema>
export type FillBlankTask = z.infer<typeof FillBlankSchema>
...
export interface TaskTypeConfig {
  ...
  schema: z.ZodSchema // Zod-схема для валидации ответа LLM
  promptInstruction: string // Инструкция для LLM как генерировать этот тип
  example: object // Пример валидного задания
  /** Подсказки по предметам — когда тип может не идеально подходить */
  subjectHints?: Partial<Record<string, string>>
}
...
const singleChoiceConfig: TaskTypeConfig = {
  id: 'single_choice',
...
const multipleChoiceConfig: TaskTypeConfig = {
  id: 'multiple_choice',
...
- options: массив из 4-6 вариантов ответа
- correctIndices: массив индексов правильных ответов (обычно 2-3)
...
const openQuestionConfig: TaskTypeConfig = {
  id: 'open_question',
...
const matchingConfig: TaskTypeConfig = {
  id: 'matching',
  name: 'Соотнесение',
...
const fillBlankConfig: TaskTypeConfig = {
  id: 'fill_blank',
...
- Количество пропусков: от 1 до 4`,
  example: {
    textWithBlanks:
      'Чтобы найти неизвестное слагаемое, надо из ___(1)___ вычесть известное ___(2)___.',
    blanks: [
      {
        position: 1,
        correctAnswer: 'суммы',
...
export const taskTypes: Record<TaskTypeId, TaskTypeConfig> = {
...
export function getTaskType(id: TaskTypeId): TaskTypeConfig {
...
export function getAllTaskTypes(): TaskTypeConfig[] {
 ...
 */
export function validateTask(typeId: TaskTypeId, task: unknown) {
 ...
 */
export function isValidTaskType(id: string): id is TaskTypeId {
