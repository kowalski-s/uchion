...
async function findUserByTelegramId(telegramId) {
    const [user] = await db
        ...
        .from(users)
        .where(and(eq(users.provider, 'telegram'), eq(users.providerId, telegramId)))
        .limit(1);
    return user || null;
}
/**
 ...
 */
async function updateAlertSubscription(userId, chatId, wantsAlerts) {
    await db
        .update(users)
        .set({
 ...
 */
async function handleSubscribe(telegramId, chatId) {
 ...
 */
async function handleUnsubscribe(telegramId) {
    const user = await findUserByTelegramId(telegramId);
    if (!user) {
        return MESSAGES.USER_NOT_FOUND;
    }
