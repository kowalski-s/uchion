...
// =============================================================================
const OPEN_MATCHING_QUOTA = {
    5: 1,
    10: 2,
    15: 3,
};
const OPEN_FILL_BLANK_QUOTA = {
    5: 1,
    10: 2,
    15: 3,
};
 ...
 */
export function distributeOpenTasks(total, selectedTypes) {
    const openTypes = selectedTypes.filter((t) => ['open_question', 'matching', 'fill_blank'].includes(t));
    if (openTypes.length === 0 || total === 0)
        return [];
    let remaining = total;
    const result = [];
    // matching quota
...
// =============================================================================
const TEST_MULTIPLE_CHOICE_QUOTA = {
    10: 3,
    15: 5,
    20: 7,
};
 ...
 */
export function distributeTestTasks(total, selectedTypes) {
    const testTypes = selectedTypes.filter((t) => ['single_choice', 'multiple_choice'].includes(t));
    if (testTypes.length === 0 || total === 0)
        return [];
    let remaining = total;
    const result = [];
 ...
 */
export function distributeAllTasks(openTotal, testTotal, selectedTypes) {
...
//# sourceMappingURL=task-distribution.js.map