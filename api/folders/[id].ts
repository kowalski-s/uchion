...
/**
 * /api/folders/[id]
 * GET    - Get single folder with worksheets
 * PATCH  - Update folder
...
async function handleGet(
  ...
  try {
    ...
    console.error('[Folders [id]] GET Error:', error)
...
async function handleUpdate(
  ...
  try {
    ...
    console.error('[Folders [id]] Update Error:', error)
    return res.status(500).json({ error: 'Internal server error' })
  }
}
...
async function handleDelete(
  ...
  try {
    ...
    console.error('[Folders [id]] Delete Error:', error)
    return res.status(500).json({ error: 'Internal server error' })
  }
}
