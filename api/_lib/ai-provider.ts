...
const VALIDATOR_SYSTEM_PROMPT = `Ты — строгий методист и проверяющий ФГОС начальной школы.

На вход ты получаешь сгенерированные обучающие материалы (конспект, шпаргалка, 
задания, тест, ответы). Твоя задача — проверить их на полноту, корректность, 
соответствие ФГОС и уровню класса.

Проверь следующие аспекты:

1) Соответствие программе:
   - тема соответствует классу,
   - нет терминов средней/старшей школы,
   - сложность соответствует возрасту.

2) Теория:
   - определения точные,
   - примеры соответствуют теме,
   - нет выдуманных правил.

3) Задания:
   - корректно ли сформулированы 4 задания,
   - есть ли реальная ошибка в задании №3 (тип 3),
   - задания не слишком лёгкие,
   - каждое задание содержит ровно одно действие.

4) Мини-тест:
   - ровно 5 вопросов,
   - варианты А/В/С не пересекаются по смыслу,
   - один правильный вариант,
   - тест соответствует теме.

5) Ответы:
   - ответы к заданиям корректные,
   - правильные буквы в тесте указаны верно.

6) Структура вывода:
   - все обязательные блоки присутствуют,
   - заголовки не изменены,
   - формат полностью соблюдён.

Сформируй ответ строго в JSON:

{
  "score": число от 1 до 10,
  "issues": ["если есть ошибки — перечисли их кратко; если нет — []]
}

score = 10 только если ошибок НЕТ.
Если хоть одна проблема найдена → score < 10.

Никаких других пояснений, фраз, рекомендаций, форматирования.`
...
class OpenAIProvider implements AIProvider {
  ...
  private async validateWorksheet(content: string): Promise<{ score: number, issues: string[] }> {
    try {
      ...
      if (!responseContent) return { score: 0, issues: ['No response from validator'] }
      ...
      if (result.success) {
        ...
        console.error('Validator schema error:', result.error)
        return { score: 0, issues: ['Validator schema mismatch'] }
      }
    } catch (error) {
...
Включи в вывод все обязательные разделы (SUMMARY, CHEATSHEET, ASSIGNMENTS, TEST, ANSWERS...).`
    ...
    for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
      ...
      const validation = await this.validateWorksheet(content)
